...
import static com.clarkparsia.pellet.utils.CollectionUtils.makeMap;
...
public class SimplifiedELClassifier extends CDOptimizedTaxonomyBuilder implements TaxonomyBuilder {
    ...
    protected void reset() {
		...
		primaryQueue = new MultiValueMap<ConceptInfo, ConceptInfo>( kb.getTBox().getAssertedAxioms().size() );
		secondaryQueue = new MultiValueMap<ConceptInfo, ConceptInfo>( kb.getTBox().getClasses().size() );
	...
	private void addToQueue(ConceptInfo sub, ConceptInfo sup) {
		if( !sub.hasSuperClass( sup ) && primaryQueue.add( sub, sup ) ) {
			if( logger.isLoggable( Level.FINER ) )
				logger.finer( "Queue1 " + sub + " " + sup );
		}
	}
	
	private void addToSecondaryQueue(ConceptInfo sub, ConceptInfo sup) {
		if( !sub.hasSuperClass( sup ) && secondaryQueue.add( sub, sup ) ) {
			if( logger.isLoggable( Level.FINER ) )
				logger.finer( "Queue2 " + sub + " - " + sup );
		}
	}

	private void addSuperClass(ConceptInfo sub, ConceptInfo sup, boolean indirect) {
		if( !sub.addSuperClass( sup, indirect ) )
			return;
		
		if( logger.isLoggable( Level.FINER ) )
			logger.finer( "Adding " + sub + " < " + sup );		
		
		for( ConceptInfo subOfSub : sub.getSubClasses() ) {
			if( !subOfSub.equals( sub ) )
				addToSecondaryQueue( subOfSub, sup );
		}
		...
		for( ConceptInfo supOfSup : sup.getSuperClasses() ) {
			if( !supOfSup.equals( sup ) )
				addToSecondaryQueue( sub, supOfSup );
		}		
	...
	private ConceptInfo createConcept(ATermAppl c) {
		...
		if( concept == null ) {
			concept = new ConceptInfo( c, hasComplexRoles, true );
	...
	private void processQueue() {
		...
		while( !primaryQueue.isEmpty() || !secondaryQueue.isEmpty() ) {
			int processed = startingSize - primaryQueue.size() - secondaryQueue.size();
	...
	private void processQueue(boolean primary) {
		MultiValueMap<ConceptInfo, ConceptInfo> localQueue;
		if( primary ) {
			localQueue = primaryQueue;
			primaryQueue = new MultiValueMap<ConceptInfo, ConceptInfo>();
		}
		else {
			localQueue = secondaryQueue;
			secondaryQueue = new MultiValueMap<ConceptInfo, ConceptInfo>();
		}
		
		for (Entry<ConceptInfo, Set<ConceptInfo>> entry : localQueue.entrySet()) {				
			ConceptInfo ci = entry.getKey();
			for (ConceptInfo sup : entry.getValue()) {
				addSuperClass( ci, sup, !primary );
			}
		}
	}
