...
//	}
	...
	public void testBooleanDatatypeConstructors() {		
		...
		createReasoner(
			...
			OWL.declaration(f),
			OWL.declaration(dq),
			OWL.range(dp, OWL.and( pi, ni ) ) 
		);
		
		assertTrue( reasoner.isSatisfiable(OWL.some( dq, pi ) ) );
		assertTrue( reasoner.isSatisfiable(OWL.some( dq, OWL.not( pi ) ) ) );
		assertFalse( reasoner.isSatisfiable(OWL.some( dq, OWL.and( pi, ni ) ) ) );
		assertFalse( reasoner.isSatisfiable(OWL.some( dq, OWL.and( f, OWL.or( pi, ni ) ) ) ) );
		assertTrue( reasoner.isSatisfiable(OWL.some( dq, OWL.and( npi, ni ) ) ) );
		assertTrue( reasoner.isSatisfiable(OWL.some( dq, OWL.and( nni, pi ) ) ) );
		assertTrue( reasoner.isSatisfiable(OWL.some( dq, OWL.or( nni, npi ) ) ) );
		assertTrue( reasoner.isSatisfiable(OWL.some( dq, OWL.and( nni, npi ) ) ) );
		assertFalse( reasoner.isSatisfiable(OWL.some( dq, OWL.and( pi, OWL.restrict( i, OWL.maxExclusive( 0 ) ) ) ) ) );
	...
	public void testGetEquivalentDataProperties() {	
		createReasoner(
			OWL.equivalentProperties(dp, dq),
			OWL.equivalentProperties(dq, dr)
		);
